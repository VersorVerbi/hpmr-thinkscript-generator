...
import {CLIArguments, HelpText, HourlyMovementData} from "./models.js";
import type {WriteStream} from "fs";
import {parseHpmrCsv} from "./functions.js";
import gatherVariables from "./prompts.js";

let code: { [key:string]: [HourlyMovementData] } = {};
...
let vars: { filename: string|null, symbol1: string, symbol2: string, createFullScript: boolean|null, outFd: WriteStream|null, useStdOut: boolean } = await gatherVariables(args);

if (!vars.filename) throw "No source CSV file provided.";
...
code[vars.symbol1] = <[HourlyMovementData]>hpmrData;
// TODO: end version 1.0-only code

for (let [symbol, data] of Object.entries(code)) {
    // TODO: fix symbols
    let scriptAdd = `    if (GetSymbol() == "${symbol}")`;
    if (vars.symbol2.length > 0) scriptAdd += ` or (GetSymbol() == "${vars.symbol2}")`;
    scriptAdd += `\n    then {\n`;
